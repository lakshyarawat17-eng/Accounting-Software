...
/* -------------------- LOCAL AUDIT / EXPORT / RECONCILIATION HELPERS -------------------- */

function safeAuditText(value, maxLen = 500) {
  if (value === null || value === undefined) return "";
  const s = typeof value === "string" ? value : JSON.stringify(value);
  return s.length > maxLen ? s.slice(0, maxLen) + "…" : s;
}

function summarizeAuditPayload(body) {
  if (!body || typeof body !== "object") return safeAuditText(body, 500);
  ...
  return safeAuditText(summary, 1500);
}

function inferAuditEntity(routePath) {
  const clean = String(routePath || "").split("?")[0].replace(/^\/+/, "");
  ...
  return map[first] || first || "system";
}

function logAuditEvent({ method, route, entity, action, statusCode, referenceNo, actor, details }) {
  return new Promise((resolve) => {
    const payload = {
      method: method || "",
      route: route || "",
      entity: entity || inferAuditEntity(route),
      action: action || method || "",
      status_code: statusCode || 0,
      reference_no: referenceNo || null,
      actor: actor || "SYSTEM",
      details: safeAuditText(details, 4000)
    };

    db.run(
      `INSERT INTO audit_log
       (event_time, method, route, entity, action, status_code, reference_no, actor, details)
       VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        payload.method,
        payload.route,
...
function normalizeBankAmount(value) {
  const n = Number(String(value || "").replace(/[, ]/g, ""));
  return Number.isFinite(n) ? n : 0;
}

function normalizeDirection(row) {
  const explicit = String(row.direction || row.side || row.type || "").toUpperCase();
  if (explicit.includes("DEBIT") || explicit.includes("DR") || explicit.includes("OUT") || explicit.includes("PAY")) return "DEBIT";
  if (explicit.includes("CREDIT") || explicit.includes("CR") || explicit.includes("IN") || explicit.includes("REC")) return "CREDIT";
  const amt = normalizeBankAmount(row.amount);
...
app.use((req, res, next) => {
  ...
  const ignored = req.path.startsWith("/webhooks/") || req.path.includes("/test-") || req.path === "/__test__";
  if (!mutating || ignored) return next();
  ...
  res.on("finish", () => {
    ...
    logAuditEvent({
      ...
      entity: inferAuditEntity(req.path),
      action: req.method,
      statusCode: res.statusCode,
      actor: req.headers["x-user-name"] || req.headers["x-user-role"] || "SYSTEM",
      details: summarizeAuditPayload(req.body)
    }).catch(() => {});
  });
  next();
});
...
app.get("/report/dashboard-summary", async (req, res) => {
  ...
  try {
    ...
    const topItems = await dbAllP(
      ...
      GROUP BY COALESCE(i.item_name, sii.description)
      ORDER BY total DESC
      LIMIT 5
      `
    );
...
/* -------------------- SALES & PURCHASE ANALYTICS --------------------
   GET /report/sales-purchase-analytics?months=6&top=10 — one aggregated
   payload for the Sales & Purchase Analytics dashboard (Sales Purchase
   Analytics.html). Unlike /report/dashboard-summary (which mixes a bit of
   everything — cash, GST position, stock, work orders — as a one-screen
   snapshot), everything here is scoped to Sales and Purchases specifically,
   and every section (except the two "current open orders" snapshot
   figures, which are a live backlog rather than a trend) is windowed to
   the same `months` lookback so the whole page reflects one selected
   period. `top` (default 10, max 25) controls how many rows each
   leaderboard (customers/suppliers/items) returns. */
app.get("/report/sales-purchase-analytics", async (req, res) => {
  const months = Math.min(Math.max(parseInt(req.query.months, 10) || 6, 1), 24);
  const top = Math.min(Math.max(parseInt(req.query.top, 10) || 10, 1), 25);
  const since = [`-${months} months`];

  try {
    // ---- Monthly sales vs purchases (+ returns, + tax) trend ----
    const salesTrendRows = await dbAllP(
      `
      SELECT strftime('%Y-%m', date) AS month,
        SUM(total_amount) AS total,
        SUM(IFNULL(cgst,0)+IFNULL(sgst,0)+IFNULL(igst,0)) AS tax
      FROM sales_invoice WHERE date >= date('now', ?) GROUP BY month
      `,
      since
    );
    const purchTrendRows = await dbAllP(
      `
      SELECT strftime('%Y-%m', date) AS month,
        SUM(total_amount) AS total,
        SUM(IFNULL(cgst,0)+IFNULL(sgst,0)+IFNULL(igst,0)) AS tax
      FROM purchase_invoice WHERE date >= date('now', ?) GROUP BY month
      `,
      since
    );
    const cnTrendRows = await dbAllP(
      `
      SELECT strftime('%Y-%m', date) AS month, SUM(total_amount) AS total
      FROM credit_note WHERE date >= date('now', ?) GROUP BY month
      `,
      since
    );
    const dnTrendRows = await dbAllP(
      `
      SELECT strftime('%Y-%m', date) AS month, SUM(total_amount) AS total
      FROM debit_note WHERE date >= date('now', ?) GROUP BY month
      `,
      since
    );
    const asMap = rows => Object.fromEntries(rows.map(r => [r.month, r]));
    const salesM = asMap(salesTrendRows), purchM = asMap(purchTrendRows);
    const cnM = asMap(cnTrendRows), dnM = asMap(dnTrendRows);
    ...
    const monthlyTrend = monthKeys.map(m => ({
      month: m,
      sales: salesM[m]?.total || 0,
      purchases: purchM[m]?.total || 0,
      sales_returns: cnM[m]?.total || 0,
      purchase_returns: dnM[m]?.total || 0,
      sales_tax: salesM[m]?.tax || 0,
      purchase_tax: purchM[m]?.tax || 0
    }));

    // ---- Headline KPIs, scoped to the same window ----
    const salesHead = await dbGetP(
      `SELECT COUNT(*) AS cnt, IFNULL(SUM(total_amount),0) AS total,
        IFNULL(SUM(IFNULL(cgst,0)+IFNULL(sgst,0)+IFNULL(igst,0)),0) AS tax,
        COUNT(DISTINCT customer) AS distinct_customers
       FROM sales_invoice WHERE date >= date('now', ?)`,
      since
    );
    const purchHead = await dbGetP(
      `SELECT COUNT(*) AS cnt, IFNULL(SUM(total_amount),0) AS total,
        IFNULL(SUM(IFNULL(cgst,0)+IFNULL(sgst,0)+IFNULL(igst,0)),0) AS tax,
        COUNT(DISTINCT supplier) AS distinct_suppliers
       FROM purchase_invoice WHERE date >= date('now', ?)`,
      since
    );
    const cnHead = await dbGetP(
      `SELECT IFNULL(SUM(total_amount),0) AS total FROM credit_note WHERE date >= date('now', ?)`,
      since
    );
    const dnHead = await dbGetP(
      `SELECT IFNULL(SUM(total_amount),0) AS total FROM debit_note WHERE date >= date('now', ?)`,
      since
    );
    const openSORow = await dbGetP(`SELECT COUNT(*) AS c FROM sales_order WHERE status NOT IN ('CLOSED','CANCELLED')`);
    const openPORow = await dbGetP(`SELECT COUNT(*) AS c FROM purchase_order WHERE status NOT IN ('CLOSED','CANCELLED')`);

    // ---- Top customers / suppliers by invoice value, within window ----
    const topCustomers = await dbAllP(
      `
      SELECT customer, COUNT(*) AS invoice_count, SUM(total_amount) AS total
      FROM sales_invoice WHERE customer IS NOT NULL AND date >= date('now', ?)
      GROUP BY customer ORDER BY total DESC LIMIT ?
      `,
      [...since, top]
    );
    const topSuppliers = await dbAllP(
      `
      SELECT supplier, COUNT(*) AS invoice_count, SUM(total_amount) AS total
      FROM purchase_invoice WHERE supplier IS NOT NULL AND date >= date('now', ?)
      GROUP BY supplier ORDER BY total DESC LIMIT ?
      `,
      [...since, top]
    );

    // ---- Top items sold / purchased by value, within window ----
    const topSalesItems = await dbAllP(
      `
      SELECT COALESCE(i.item_name, sii.description) AS item_name,
        SUM(sii.qty) AS qty, SUM(sii.total) AS total
      FROM sales_invoice_items sii
      JOIN sales_invoice si ON si.id = sii.invoice_id
      LEFT JOIN item_master i ON i.id = sii.item_id
      WHERE si.date >= date('now', ?)
      GROUP BY COALESCE(i.item_name, sii.description) ORDER BY total DESC LIMIT ?
      `,
      [...since, top]
    );
    const topPurchaseItems = await dbAllP(
      `
      SELECT COALESCE(i.item_name, pii.item_name) AS item_name,
        SUM(pii.qty) AS qty, SUM(pii.total) AS total
      FROM purchase_invoice_items pii
      JOIN purchase_invoice pi ON pi.id = pii.invoice_id
      LEFT JOIN item_master i ON i.id = pii.item_id
      WHERE pi.date >= date('now', ?)
      GROUP BY COALESCE(i.item_name, pii.item_name) ORDER BY total DESC LIMIT ?
      `,
      [...since, top]
    );

    // ---- GST-rate-wise breakup of sales & purchase lines, within window ----
    const gstRateSales = await dbAllP(
      `
      SELECT IFNULL(sii.gst_rate,0) AS gst_rate,
        SUM(sii.taxable) AS taxable, SUM(sii.gst_amount) AS gst_amount, SUM(sii.total) AS total
      FROM sales_invoice_items sii
      JOIN sales_invoice si ON si.id = sii.invoice_id
      WHERE si.date >= date('now', ?)
      GROUP BY gst_rate ORDER BY gst_rate ASC
      `,
      since
    );
    const gstRatePurchase = await dbAllP(
      `
      SELECT IFNULL(pii.gst_rate,0) AS gst_rate,
        SUM(pii.taxable) AS taxable, SUM(pii.gst_amount) AS gst_amount, SUM(pii.total) AS total
      FROM purchase_invoice_items pii
      JOIN purchase_invoice pi ON pi.id = pii.invoice_id
      WHERE pi.date >= date('now', ?)
      GROUP BY gst_rate ORDER BY gst_rate ASC
    ...
    // ---- Sales order status breakdown + fulfillment funnel, within window ----
    const soStatus = await dbAllP(
      `SELECT status, COUNT(*) AS count FROM sales_order WHERE date >= date('now', ?) GROUP BY status`,
      since
    );
    const soFunnelRow = await dbGetP(
      `
      SELECT
        IFNULL(SUM(soi.qty * soi.rate),0) AS ordered_value,
        IFNULL(SUM(IFNULL(soi.delivered_qty,0) * soi.rate),0) AS delivered_value,
        IFNULL(SUM(IFNULL(soi.invoiced_qty,0) * soi.rate),0) AS invoiced_value
      FROM sales_order_items soi
      JOIN sales_order so ON so.id = soi.so_id
      WHERE so.date >= date('now', ?)
      `,
      since
    );

    // ---- Purchase order status breakdown + fulfillment funnel, within window ----
    const poStatus = await dbAllP(
      `SELECT status, COUNT(*) AS count FROM purchase_order WHERE date >= date('now', ?) GROUP BY status`,
      since
    );
    const poFunnelRow = await dbGetP(
      `
      SELECT
        IFNULL(SUM(poi.qty * poi.rate),0) AS ordered_value,
        IFNULL(SUM(IFNULL(poi.received_qty,0) * poi.rate),0) AS received_value,
        IFNULL(SUM(IFNULL(poi.invoiced_qty,0) * poi.rate),0) AS invoiced_value
      FROM purchase_order_items poi
      JOIN purchase_order po ON po.id = poi.po_id
      WHERE po.date >= date('now', ?)
      `,
      since
    );
    ...
    res.json({
      period_months: months,
      kpis: {
        total_sales: salesHead?.total || 0,
        total_purchases: purchHead?.total || 0,
        sales_invoice_count: salesHead?.cnt || 0,
        purchase_invoice_count: purchHead?.cnt || 0,
        avg_sale_value: salesHead?.cnt ? salesHead.total / salesHead.cnt : 0,
        avg_purchase_value: purchHead?.cnt ? purchHead.total / purchHead.cnt : 0,
        sales_returns_total: cnHead?.total || 0,
        purchase_returns_total: dnHead?.total || 0,
        net_sales: (salesHead?.total || 0) - (cnHead?.total || 0),
        net_purchases: (purchHead?.total || 0) - (dnHead?.total || 0),
        sales_tax_collected: salesHead?.tax || 0,
        purchase_tax_credit: purchHead?.tax || 0,
        active_customers: salesHead?.distinct_customers || 0,
        active_suppliers: purchHead?.distinct_suppliers || 0,
        open_sales_orders: openSORow?.c || 0,
        open_purchase_orders: openPORow?.c || 0
      },
      monthly_trend: monthlyTrend,
      top_customers: topCustomers,
      top_suppliers: topSuppliers,
      top_sales_items: topSalesItems,
      top_purchase_items: topPurchaseItems,
      gst_rate_breakup_sales: gstRateSales,
      gst_rate_breakup_purchase: gstRatePurchase,
      sales_order_status: soStatus,
      purchase_order_status: poStatus,
      sales_order_funnel: soFunnelRow || { ordered_value: 0, delivered_value: 0, invoiced_value: 0 },
      purchase_order_funnel: poFunnelRow || { ordered_value: 0, received_value: 0, invoiced_value: 0 }
    });
  } catch (err) {
    console.error("SALES PURCHASE ANALYTICS ERROR:", err.message);
    res.status(500).json({ error: err.message });
  }
});


/* -------------------- AUDIT TRAIL / BACKUP / BANK RECONCILIATION -------------------- */
...
app.get("/audit/logs", async (req, res) => {
  try {
    ...
    if (q) {
      where.push("(route LIKE ? OR actor LIKE ? OR reference_no LIKE ? OR details LIKE ?)");
      const like = `%${q}%`;
...
app.get("/audit/export.csv", async (req, res) => {
  try {
    const rows = await dbAllAsync(
      `SELECT event_time, method, route, entity, action, status_code, reference_no, actor, details
       FROM audit_log
       ORDER BY id DESC`
    );
...
app.get("/exports/:entity.csv", async (req, res) => {
  try {
    ...
    if (entity === "ledgers") {
      rows = await dbAllAsync(
        `SELECT lm.ledger, lm.ledger_group, lg.nature, lm.is_system
         FROM ledger_master lm
         LEFT JOIN ledger_group_master lg ON lm.ledger_group = lg.group_name
         ORDER BY lm.ledger`
      );
      headers = ["ledger", "ledger_group", "nature", "is_system"];
    } else if (entity === "journals") {
      rows = await dbAllAsync(
        `SELECT jv.voucher_no, jv.date, jv.narration,
                je.ledger, je.lf, je.debit, je.credit
         FROM journal_voucher jv
         JOIN journal_entries je ON je.voucher_id = jv.id
         ORDER BY jv.id DESC, je.id ASC`
      );
      headers = ["voucher_no", "date", "narration", "ledger", "lf", "debit", "credit"];
    } else if (entity === "payments") {
      rows = await dbAllAsync(
        `SELECT voucher_no, type, date, party, mode_ledger, amount, narration, journal_voucher_no
         FROM payment_voucher
         ORDER BY id DESC`
      );
      headers = ["voucher_no", "type", "date", "party", "mode_ledger", "amount", "narration", "journal_voucher_no"];
      ...
      headers = ["event_time", "method", "route", "entity", "action", "status_code", "reference_no", "actor", "details"];
    } else if (entity === "customers") {
      rows = await dbAllAsync(
        `SELECT id, client_name, gstin, phone, email, state, created_at
         FROM clients
         ORDER BY client_name`
      );
      headers = ["id", "client_name", "gstin", "phone", "email", "state", "created_at"];
    } else if (entity === "suppliers") {
      rows = await dbAllAsync(
        `SELECT id, supplier_name, gstin, phone, email, state, created_at
         FROM suppliers
         ORDER BY supplier_name`
      );
      headers = ["id", "supplier_name", "gstin", "phone", "email", "state", "created_at"];
    } else if (entity === "items") {
      rows = await dbAllAsync(
        `SELECT id, item_code, item_name, hsn, unit, gst_rate, selling_price, opening_qty, opening_rate, supply_type, item_type
         FROM item_master
         ORDER BY item_name`
      );
      headers = ["id", "item_code", "item_name", "hsn", "unit", "gst_rate", "selling_price", "opening_qty", "opening_rate", "supply_type", "item_type"];
    } else if (entity === "locations") {
      rows = await dbAllAsync(
        `SELECT id, location_name, location_type, is_active
         FROM locations
         ORDER BY location_name`
      );
      headers = ["id", "location_name", "location_type", "is_active"];
    } else {
...
app.get("/backup/download", (req, res) => {
  try {
    const backupName = `accounts-backup-${new Date().toISOString().slice(0, 10)}.db`;
    res.download(DB_FILE, backupName);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.post("/bank-reconciliation/import", async (req, res) => {
  try {
    const bankLedger = String(req.body.bankLedger || "").trim();
    const statementName = String(req.body.statementName || "").trim();
    const statementDate = String(req.body.statementDate || "").trim() || null;
    const csvText = String(req.body.csvText || "");
    const incomingRows = Array.isArray(req.body.rows) ? req.body.rows : [];

    if (!bankLedger) {
      return res.status(400).json({ error: "Bank ledger is required" });
    }

    let rows = incomingRows;
    if (!rows.length && csvText.trim()) {
      rows = parseStatementCsv(csvText);
    }

    if (!rows.length) {
      return res.status(400).json({ error: "No statement rows supplied" });
    }
    ...
    const header = await new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO bank_statement_import (bank_ledger, statement_name, statement_date, source_type)
         VALUES (?, ?, ?, 'CSV')`,
        [bankLedger, statementName || `Imported statement`, statementDate],
        function (err) {
          if (err) return reject(err);
          resolve(this.lastID);
        }
    ...
    for (const raw of rows) {
      const txnDate = String(raw.date || raw.txn_date || raw.transaction_date || raw.value_date || statementDate || "").trim() || null;
      const description = String(raw.description || raw.narration || raw.particulars || raw.details || "").trim();
      const referenceNo = String(raw.reference || raw.ref || raw.cheque_no || raw.chq_no || raw.utr || "").trim();
      const amount = normalizeBankAmount(raw.amount || raw.txn_amount || raw.debit || raw.credit);
      ...
      const candidates = await dbAllAsync(
        `
        SELECT id, voucher_no, type, date, party, amount, narration
        FROM payment_voucher
        WHERE mode_ledger = ?
          AND ABS(amount - ?) < 0.01
          AND date BETWEEN date(?, '-7 day') AND date(?, '+7 day')
        ORDER BY ABS(julianday(date) - julianday(?)), id DESC
        `,
        [bankLedger, absAmount, txnDate || statementDate || "now", txnDate || statementDate || "now", txnDate || statementDate || "now"]
      );

      const match = candidates.find(c => !usedPaymentIds.has(c.id) && (
        (direction === "DEBIT" && c.type === "PAYMENT") ||
        (direction === "CREDIT" && c.type === "RECEIPT")
      ));
      ...
      if (match) {
        ...
        await dbRunAsync(
          `INSERT INTO bank_statement_line
           (import_id, txn_date, description, reference_no, amount, direction, matched_type, matched_id, matched_no, status, raw_json)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'MATCHED', ?)`,
          [header, txnDate, description, referenceNo, signedAmount, direction, match.type, match.id, match.voucher_no, JSON.stringify(raw)]
        );
      } else {
        unmatched += 1;
        await dbRunAsync(
          `INSERT INTO bank_statement_line
           (import_id, txn_date, description, reference_no, amount, direction, status, raw_json)
           VALUES (?, ?, ?, ?, ?, ?, 'UNMATCHED', ?)`,
          [header, txnDate, description, referenceNo, signedAmount, direction, JSON.stringify(raw)]
        );
      }
    }
    ...
    await logAuditEvent({
      method: "POST",
      route: "/bank-reconciliation/import",
      entity: "bank-reconciliation",
      action: "IMPORT",
      statusCode: 200,
      referenceNo: String(header),
      details: { bankLedger, statementName, rows: rows.length, matched, unmatched }
    });
...
app.get("/bank-reconciliation/list", async (req, res) => {
  try {
    const bankLedger = String(req.query.bankLedger || "").trim();
    const where = bankLedger ? "WHERE i.bank_ledger = ?" : "";
    const params = bankLedger ? [bankLedger] : [];
    const rows = await dbAllAsync(
      `
      SELECT
        i.*,
        COUNT(l.id) AS total_rows,
        SUM(CASE WHEN l.status = 'MATCHED' THEN 1 ELSE 0 END) AS matched_rows,
        SUM(CASE WHEN l.status = 'UNMATCHED' THEN 1 ELSE 0 END) AS unmatched_rows
      FROM bank_statement_import i
      LEFT JOIN bank_statement_line l ON l.import_id = i.id
      ${where}
      GROUP BY i.id
      ORDER BY i.id DESC
      `,
...
app.get("/bank-reconciliation/:id", async (req, res) => {
  try {
    const id = Number(req.params.id);
    const header = await dbGetAsync(`SELECT * FROM bank_statement_import WHERE id = ?`, [id]);
    if (!header) {
      return res.status(404).json({ error: "Import not found" });
    }
    const lines = await dbAllAsync(
      `SELECT *
