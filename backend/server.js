...
/* Outstanding sales invoices (optionally filtered to one customer) — each
   ...
   from payment_allocation so far. Only invoices with balance > 0 are
   returned, oldest first, which also makes this ready to drive a
   "settle oldest first" default on the Receipt screen. */
app.get("/receivables/outstanding", (req, res) => {
  ...
  db.all(
    ...
    SELECT
      ...
      IFNULL(SUM(pa.allocated_amount), 0) AS paid,
      si.total_amount - IFNULL(SUM(pa.allocated_amount), 0) AS balance
    FROM sales_invoice si
    LEFT JOIN payment_allocation pa
      ON pa.invoice_type = 'SALES' AND pa.invoice_id = si.id
...
/* Same idea for purchase invoices (payables). */
app.get("/payables/outstanding", (req, res) => {
  ...
  db.all(
    ...
    SELECT
      ...
      IFNULL(SUM(pa.allocated_amount), 0) AS paid,
      pi.total_amount - IFNULL(SUM(pa.allocated_amount), 0) AS balance
    FROM purchase_invoice pi
    LEFT JOIN payment_allocation pa
      ON pa.invoice_type = 'PURCHASE' AND pa.invoice_id = pi.id
    ${supplier ? "WHERE pi.supplier = ?" : ""}
    GROUP BY pi.id
    HAVING balance > 0.005
    ORDER BY pi.date, pi.id
    `,
    supplier ? [supplier] : [],
    (err, rows) => (err ? res.status(500).json({ error: err.message }) : res.json(rows))
  );
...
app.post("/payment/save", async (req, res) => {
  ...
  try {
    ...
    for (const a of allocList) {
      ...
      const outstanding = invoice.total_amount - alreadyPaid;
