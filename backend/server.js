...
/* -------------------- HSN/SAC VALIDATION --------------------
   Per CBIC Notification 78/2020 (in force since 01-Apr-2021), the number
   of HSN digits a taxpayer must declare on tax invoices depends on prior
   financial year turnover:
     - Turnover > Rs 5 crore        -> 6-digit HSN mandatory on ALL invoices
     - Turnover Rs 1.5cr - Rs 5cr   -> 4-digit HSN mandatory (B2B); we apply
                                        this at the item-master level since
                                        a single item is sold to both B2B
                                        and B2C customers
     - Turnover < Rs 1.5 crore      -> HSN optional, but if entered it must
                                        still be a valid 4/6/8-digit code
   We store the declared annual turnover as a setting (`annual_turnover`,
   in rupees) and derive the required digit length from it. HSN/SAC codes
   are always purely numeric with length 2, 4, 6, or 8 (services use SAC,
   which is also numeric and conventionally 6 digits under Chapter 99). */
...
// Returns 0 if HSN is optional (turnover below Rs 1.5 crore or not set).
function requiredHsnDigits(annualTurnover) {
  const t = Number(annualTurnover) || 0;
  if (t > 50000000) return 6;   // > Rs 5 crore
  if (t > 15000000) return 4;   // Rs 1.5 crore - Rs 5 crore
  return 0;                     // below Rs 1.5 crore -> optional
}
...
function validateHsnForTurnover(hsn, annualTurnover) {
  ...
  if (!trimmed) {
    if (minDigits > 0) {
      return {
        ok: false,
        error: `HSN/SAC code is required (your declared turnover requires a minimum ${minDigits}-digit HSN on every item, per CBIC Notification 78/2020).`
      };
    }
    return { ok: true }; // optional and left blank
  }
  ...
  if (minDigits > 0 && trimmed.length < minDigits) {
    return {
      ok: false,
...
function renderGstTabularTemplate(doc, { invoiceNo, date, customer, items, amount, gstBreakup, grandTotal, seller, buyer }) {
...
async function generateSalesInvoicePDF({ invoiceNo, date, customer, items, amount, gstBreakup, grandTotal, template, seller, buyer }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    renderer(doc, { invoiceNo, date, customer, items, amount, gstBreakup, grandTotal, seller, buyer });
...
app.put("/item/:id", async (req, res) => {
  ...
  db.run(
    `
    UPDATE item_master
    SET item_code = ?, item_name = ?, hsn = ?, unit = ?, gst_rate = ?, selling_price = ?
    WHERE id = ?
    `,
    [item_code, item_name, hsn, unitUpper, gst_rate, selling_price || 0, itemId],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
...
app.post("/purchase/save", async (req, res) => {
  const { date, supplier, invoiceNo, items, taxType, po_id, supplierId } = req.body;
...
app.post("/debit-note/save", async (req, res) => {
  const { date, supplier, purchase_invoice_id, reason, taxType, adjustsStock, items, supplierId } = req.body;
...
app.get("/report/hsn-summary", (req, res) => {
  ...
  const fetchLines = (itemsTable, headerTable, headerIdCol, dateCol, nameCol, hasHsnCol, netSign) => new Promise((resolve, reject) => {
    db.all(
      `
      SELECT
  ...
  Promise.all([
    fetchLines("sales_invoice_items", "sales_invoice", "invoice_id", "date", "description", true, 1),
    fetchLines("credit_note_items", "credit_note", "note_id", "date", "item_name", false, -1)
  ])
    .then(([salesLines, creditLines]) => {
      const allLines = salesLines.concat(creditLines);
      const summary = {}; // hsn -> aggregated row
      ...
      res.json({
        ...
        rows,
        totals,
        has_missing_hsn: rows.some(r => r.missing_hsn)
      });
    })
    .catch(err => res.status(500).json({ error: err.message }));
});
