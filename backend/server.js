...
function computeHsnSummary(month) {
  ...
  Promise.all([
    ...
    .then(([salesLines, creditLines]) => {
      ...
      // Two independent aggregations, one per GSTR-1 Table 12 tab.
      const b2bSummary = {};
      const b2cSummary = {};
      ...
      allLines.forEach(row => {
        ...
        const summary = isB2B ? b2bSummary : b2cSummary;
      ...
      // plus an overall totals/missing-HSN flag across both tabs.
      ...
      resolve({
        b2b,   // GSTR-1 Table 12A — every registered buyer, mandatory regardless of turnover
        b2c,   // GSTR-1 Table 12B — unregistered buyers, mandatory only above Rs 5cr AATO
        rows: combinedRows,
        totals: combinedTotals,
        has_missing_hsn: b2b.has_missing_hsn || b2c.has_missing_hsn
      });
    })
    .catch(reject);
  });
...
function buildGstr1(month) {
  ...
  return Promise.all([
    ...
    dbAll(
      `SELECT sii.invoice_id, sii.gst_rate, sii.taxable, sii.gst_amount
       FROM sales_invoice_items sii
    ...
    dbAll(
      `SELECT cni.note_id, cni.gst_rate, cni.taxable, cni.gst_amount
    ...
    computeHsnSummary(month)
  ]).then(([rawGstin, salesHeaders, salesItems, creditHeaders, creditItems, hsnResult]) => {
