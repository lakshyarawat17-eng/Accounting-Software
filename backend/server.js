...
app.post("/item/create", async (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run(
      `
      INSERT INTO item_master
      (item_code, item_name, hsn, unit, gst_rate, selling_price, opening_qty, opening_rate, supply_type)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [
        item_code,
...
app.put("/item/:id", async (req, res) => {
  ...
  db.run(
    `
    UPDATE item_master
    SET item_code = ?, item_name = ?, hsn = ?, unit = ?, gst_rate = ?, selling_price = ?, supply_type = ?
    WHERE id = ?
    `,
    [item_code, item_name, hsn, unitUpper, effectiveGstRate, selling_price || 0, supplyType, itemId],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
...
function buildGstr1(month) {
  ...
  return Promise.all([
    ...
    const groupExemptNilByDoc = (rows, keyCol) => {
      const byDoc = {};
      rows.forEach(r => {
        const cat = normalizeSupplyType(r.supply_type) || "TAXABLE";
    ...
    const salesExemptNilByInvoice = groupExemptNilByDoc(salesItems, "invoice_id");
    const creditExemptNilByNote = groupExemptNilByDoc(creditItems, "note_id");

    const nilBuckets = {}; // sply_ty -> { expt_amt, nil_amt }
    const addToNil = (headers, itemsByDoc, sign) => {
      headers.forEach(doc => {
        const rows = itemsByDoc[doc.id];
        ...
        rows.forEach(r => {
          const cat = normalizeSupplyType(r.supply_type);
          const amt = sign * (Number(r.taxable) || 0);
          if (cat === "EXEMPT") nilBuckets[sply_ty].expt_amt += amt;
          else if (cat === "NIL_RATED") nilBuckets[sply_ty].nil_amt += amt;
        });
      });
    };
    addToNil(salesHeaders, salesExemptNilByInvoice, 1);
    addToNil(creditHeaders, creditExemptNilByNote, -1);
