...
// `credentials: true` is required for the RBAC login cookie (rbac/auth.js) to
// be sent/received cross-origin — note this also means, once FRONTEND_URL is
// unset, the reflected "allow any origin" fallback effectively allows any site
// to make credentialed requests, so set FRONTEND_URL in production.
...
function saveJournalInternal({ date, narration, entries, userId }) {
  return new Promise((resolve, reject) => {
    ...
    db.all(
      ...
      (err, rows) => {
        ...
        getNextVoucherNo((err, voucherNo) => {
          ...
          db.run(
            `INSERT INTO journal_voucher (date, voucher_no, narration, created_by)
             VALUES (?, ?, ?, ?)`,
            [date, voucherNo, narration, userId || null],
            function (err) {
              if (err) return reject(err);
...
/* -------------------- RBAC (Steps 2 & 3) --------------------
   Step 2: create roles/role_permissions/users/sessions tables and seed
   them from rbac/permission-matrix.js (Step 1), plus a one-time default
   Administrator account.
   Step 3: identify the caller on every request via req.user (attachUser)
   and expose POST /auth/login, POST /auth/logout, GET /auth/me.
   Nothing here rejects a request yet — that's Step 5. */
const { ensureRbacTables } = require("./rbac/db-schema");
const { seedRoles, ensureDefaultAdmin, ensureFixedAdmin } = require("./rbac/seed");
const { attachUser, mountAuthRoutes } = require("./rbac/auth");
const { requirePermission } = require("./rbac/enforce");
const { mountUserRoutes } = require("./rbac/users-routes");
const { ensureAuditTable, ensureOwnershipColumns, auditMiddleware } = require("./rbac/audit");
const { ensureRolloutTable } = require("./rbac/rollout");
const { mountRolloutRoutes } = require("./rbac/rollout-routes");

// ensureRolloutTable runs before seedRoles: Step 10's isModulePermissive()
// guard (used inside seedRoles) queries the rbac_rollout table, so it
// needs to exist first. Harmless no-op on every boot after the first.
ensureRbacTables(db)
  .then(() => ensureRolloutTable(db))
  .then(() => seedRoles(db))
  .then(() => ensureDefaultAdmin(db))
  // Guarantees admin@login.com / 1234 always exists as an active
  // Administrator, on every boot, even against a DB that already has
  // users or predates RBAC entirely. See the security warning in
  // rbac/seed.js — change/remove this before going live publicly.
  .then(() => ensureFixedAdmin(db))
  .then(() => ensureAuditTable(db))
  .then(() => ensureOwnershipColumns(db))
  .catch(err => console.error("RBAC setup failed:", err.message));

app.use(attachUser(db));
mountAuthRoutes(app, db);

/* Step 4: enforcement. Ships in dry-run (log-only) by default — see the
   big comment at the top of rbac/enforce.js before setting RBAC_ENFORCE=true. */
app.use(requirePermission(db));

/* Step 9: audit logging. Reads req.user (Step 3) + req.rbac (Step 4), set
   by the two middlewares above, so this must be registered after both.
   Purely observational — never blocks a request. See rbac/audit.js. */
app.use(auditMiddleware(db));

/* Step 8: /users + /roles (Users.html admin screen). Registered after
   requirePermission() above so these routes get the same admin_settings
   gate as everything else — route-module-map.js already maps both
   prefixes to "admin_settings". */
mountUserRoutes(app, db);

/* Step 10: staged rollout controls (/rbac/rollout/*) — same admin_settings
   gate as /users above. See rbac/rollout.js for what these do. */
mountRolloutRoutes(app, db);
...
app.post("/sales/save", async (req, res) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    await saveJournalInternal({ userId: req.user ? req.user.id : null,
      date,
...
app.post("/asset/:id/dispose", async (req, res) => {
  ...
  try {
    ...
    if (stubCharge > 0) {
      stubVoucherNo = await saveJournalInternal({ userId: req.user ? req.user.id : null,
        date,
    ...
    if (entries.length) {
      disposalVoucherNo = await saveJournalInternal({ userId: req.user ? req.user.id : null,
        date,
...
app.post("/payment/save", async (req, res) => {
  ...
  try {
    ...
    const journalVoucherNo = await saveJournalInternal({ userId: req.user ? req.user.id : null,
      date,
    ...
    const paymentId = await new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO payment_voucher (voucher_no, type, date, party, mode_ledger, amount, narration, journal_voucher_no, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [voucherNo, type, date, party, mode_ledger, Number(amount), narration || null, journalVoucherNo, req.user ? req.user.id : null],
        function (err) {
          if (err) return reject(err);
          resolve(this.lastID);
        }
