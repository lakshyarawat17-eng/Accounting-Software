...
/* -------------------- EMAIL (HTTP API, not SMTP) --------------------
   ...
   mail over a normal HTTPS API call. Two providers are supported —
   ...
   • Brevo    — needs a verified "from" sender (no domain needed) but
     ...
     your own inbox. Good once you're past initial testing.
     Setup: https://www.brevo.com -> Senders -> verify one address,
     then Settings -> SMTP & API -> API Keys -> Generate
     Railway env vars: BREVO_API_KEY (+ mail_from_email/mail_from_name
     from Settings, or MAIL_FROM_EMAIL/MAIL_FROM_NAME env vars)

   Whichever key is present in env vars (or saved in Settings) is the
   one that gets used. If both are set, Resend wins.
------------------------------------------------------------------ */
...
async function sendEmail({ to, subject, text, attachments = [] }) {
  ...
  throw new Error(
    "Email is not configured yet. Add a Resend or Brevo API key in Settings (or as a Railway env var: RESEND_API_KEY / BREVO_API_KEY)."
  );
}
