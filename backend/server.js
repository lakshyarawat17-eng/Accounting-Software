...
function logWhatsAppAttempt({ invoice_no, client_id, phone, status, error }) {
  db.run(
    `INSERT INTO whatsapp_log (invoice_no, client_id, phone, status, error)
     VALUES (?, ?, ?, ?, ?)`,
    [invoice_no || null, client_id || null, phone || null, status, error || null]
  );
}
...
// Send a quick test WhatsApp message to confirm the Twilio settings work.
app.post("/settings/test-whatsapp", async (req, res) => {
  ...
  try {
    ...
    res.json({ status: "success", sentTo: phone });
  } catch (err) {
...
app.post("/invoices/send-whatsapp", async (req, res) => {
  ...
  try {
    ...
    const normalizedPhone = normalizePhoneForWhatsApp(recipientPhone, cfg.whatsapp_default_country_code);
    const mediaUrl = cfg.public_base_url
      ? `${cfg.public_base_url.replace(/\/$/, "")}/invoices/sales/${invoiceNo}.pdf`
      : null;

    // Invoice notifications are business-initiated (the client hasn't just
    // messaged us), so WhatsApp requires a Content Template rather than a
    // free-form Body. In the Sandbox we're limited to Twilio's pre-approved
    // "Order Notifications" template: "Your {{1}} order of {{2}} has shipped
    // and should be delivered on {{3}}. Details: {{4}}" - repurposed here
    // for testing. Swap this mapping for your own template's fields once
    // you register a real WhatsApp Sender for production.
    await sendWhatsApp({
      to: normalizedPhone,
      contentVariables: {
        "1": "Invoice",
        "2": invoiceNo,
        "3": new Date().toLocaleDateString("en-IN"),
        "4": message || mediaUrl || `Invoice ${invoiceNo}`
      }
    });
    ...
    res.json({ status: "success", sentTo: normalizedPhone });
  } catch (err) {
