...
async function getSmtpConfig() {
  const values = await Promise.all(SMTP_SETTING_KEYS.map(k => getSetting(k, "")));
  const cfg = {};
  SMTP_SETTING_KEYS.forEach((k, i) => (cfg[k] = values[i]));
  return cfg;
}
...
async function getMailTransporter() {
  ...
  if (!cfg.smtp_host || !cfg.smtp_port || !cfg.smtp_user || !cfg.smtp_pass) {
    throw new Error(
      "Email is not configured yet. Go to Settings and fill in your SMTP details first."
    );
  }
  ...
  const transporter = nodemailer.createTransport({
    ...
    // Many cloud hosts (Railway, Docker, etc.) have no outbound IPv6 route.
    // Without this, Node may pick Gmail's IPv6 address and fail with
    // ENETUNREACH. Forcing IPv4 avoids that.
    family: 4
  });

  const fromEmail = cfg.smtp_from_email || cfg.smtp_user;
  const fromName = cfg.smtp_from_name || "Accounts";
