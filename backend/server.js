...
app.post("/item/create", async (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run(
      `
      INSERT INTO item_master
      (item_code, item_name, hsn, unit, gst_rate, selling_price, opening_qty, opening_rate, supply_type, item_type, secondary_unit, conversion_factor)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [
        item_code,
...
app.put("/item/:id", async (req, res) => {
  ...
  db.run(
    `
    UPDATE item_master
    SET item_code = ?, item_name = ?, hsn = ?, unit = ?, gst_rate = ?, selling_price = ?, supply_type = ?, item_type = ?, secondary_unit = ?, conversion_factor = ?
    WHERE id = ?
    `,
    [item_code, item_name, hsn, unitUpper, effectiveGstRate, selling_price || 0, supplyType, itemType, secondaryUnit, conversionFactor, itemId],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
    ...
    for (const line of items) {
      ...
      await new Promise((resolve, reject) => {
        db.run(
          ...
          VALUES (?, ?, 'GRN', ?, ?, ?, ?)
          `,
          [poItem.item_id, date, po.po_no, qty, poItem.rate, grnLocationId],
          err => (err ? reject(err) : resolve())
        );
      });
...
app.get("/report/item/:itemId", (req, res) => {
  ...
  const getSummary = () => new Promise((resolve, reject) => {
    db.get(
      ...
      FROM item_master i
      LEFT JOIN stock_ledger s ON i.id = s.item_id ${locationId ? "AND s.location_id = ?" : ""}
      WHERE i.id = ?
      `,
  ...
  const getMovements = () => new Promise((resolve, reject) => {
    db.all(
      ...
      WHERE s.item_id = ? ${locationId ? "AND s.location_id = ?" : ""}
  ...
  Promise.all([getSummary(), getMovements(), getByLocation(), getUnassigned()])
    .then(([summary, movements, by_location, unassigned]) => {
