...
app.post("/payment/save", async (req, res) => {
  ...
  try {
    ...
    for (const a of allocList) {
      ...
      // Debit notes only apply to purchase invoices (no credit-note
      // equivalent on the sales side yet), and reduce what's owed just
      // like a payment allocation would.
      ...
      const outstanding = invoice.total_amount - alreadyPaid - alreadyDebited;
