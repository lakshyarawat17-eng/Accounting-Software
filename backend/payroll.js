...
module.exports = function registerPayroll(app, db, helpers) {
  ...
  app.post("/hr/payroll/run/create", async (req, res) => {
    ...
    try {
      ...
      if (existing) {
        ...
        const r = await dbRun(`INSERT INTO payroll_run (month, year, status) VALUES (?,?,'DRAFT')`, [month, year]);
        runId = r.lastID;
      }
  ...
  app.post("/hr/payroll/run/:id/process", async (req, res) => {
    try {
      ...
      await dbRun(`UPDATE payroll_run SET status = 'PROCESSED', journal_voucher_no = ?, processed_at = datetime('now') WHERE id = ?`, [voucherNo, req.params.id]);
  ...
  app.post("/hr/payroll/run/:id/mark-paid", async (req, res) => {
    ...
    try {
      ...
      await dbRun(`UPDATE payroll_run SET status = 'PAID', payment_voucher_no = ?, paid_at = datetime('now') WHERE id = ?`, [voucherNo, req.params.id]);
