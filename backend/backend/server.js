...
app.post("/settings/test-whatsapp", async (req, res) => {
  ...
  try {
    ...
    await sendWhatsApp({
      to: phone,
      body: "This is a test WhatsApp message. If you received this, your WhatsApp settings are working correctly."
    });
...
app.post("/invoices/send-whatsapp", async (req, res) => {
  ...
  try {
    ...
    const mediaUrl = `${cfg.public_base_url.replace(/\/$/, "")}/invoices/sales/${invoiceNo}.pdf`;

    await sendWhatsApp({
      to: normalizedPhone,
      body: message || `Please find attached invoice ${invoiceNo}. Thank you for your business.`,
      mediaUrl
    });
