...
/* -------------------- EMAIL (Brevo HTTP API) --------------------
   ...
   which is why nodemailer/SMTP never worked here. Brevo (formerly
   Sendinblue) sends email over a normal HTTPS API call instead, so it
   works fine on Railway. Free tier: 300 emails/day, no credit card,
   and — unlike most HTTP email APIs — you don't need to own/verify a
   domain to send to real client addresses, just verify your one
   "from" address by clicking the link Brevo emails you.

   Setup:
   1. Create a free account at https://www.brevo.com
   2. Verify a "from" sender email (Senders, Domains & Dedicated IPs ->
      Senders -> Add a sender). Use any inbox you control.
   3. Create an API key (Settings -> SMTP & API -> API Keys -> Generate).
   4. Either:
      a) Railway -> your service -> Variables -> add BREVO_API_KEY, or
      b) paste the key into Settings -> Email in this app (stored in
         the settings table). The env var wins if both are set.
------------------------------------------------------------------ */

const MAIL_SETTING_KEYS = ["brevo_api_key", "mail_from_name", "mail_from_email"];

async function getMailConfig() {
  const values = await Promise.all(MAIL_SETTING_KEYS.map(k => getSetting(k, "")));
  const cfg = {};
  MAIL_SETTING_KEYS.forEach((k, i) => (cfg[k] = values[i]));
  // Env var takes priority so the key doesn't have to live in the DB.
...
async function sendEmail({ to, subject, text, attachments = [] }) {
  ...
  if (!cfg.brevo_api_key) {
    throw new Error(
      "Email is not configured yet. Go to Settings and paste in your Brevo API key first (or set BREVO_API_KEY in Railway)."
    );
  }
