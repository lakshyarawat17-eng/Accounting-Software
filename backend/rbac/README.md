...
- **Step 4** — the actual enforcement middleware. Now **fail-closed by
  default**: it enforces unless `RBAC_ENFORCE=false` is set explicitly.
  See "Before you disable Step 4" below.
...
## ⚠️ Before you disable Step 4

The frontend gate (`frontend/auth.js` + `Login.html`, Step 5 above) is
built and live, and Step 2's `ensureDefaultAdmin()` guarantees an
Administrator account exists from the very first boot. There's no longer
a bootstrapping reason to ship with enforcement off, so:

- **Enforcement is ON by default.** Unset, missing, or misspelled
  `RBAC_ENFORCE` all mean *enforcing* — unauthenticated or unauthorized
  requests get a real 401/403. This is a deliberate fail-closed change:
  previously an unset/lost env var silently disabled access control on
  every route (including payroll and user management); now it fails
  safe instead.
- **Set `RBAC_ENFORCE=false`** only to deliberately run in dry-run/log-only
  mode — e.g. briefly in local development before real accounts exist.
  In that mode the server still resolves and logs `RBAC [DRY RUN] ...
  would block` for every request, but nothing is actually blocked. Never
  leave this set in a real deployment.
- **Rolling out a changed `permission-matrix.js` against real users**
  without a hard cutover that locks people out on day one? Use Step 10's
  staged rollout (`startPermissiveRollout()` then `tightenModule()` per
  module — see "Step 10: Staged Rollout" below) instead of dry-run mode.
  That's the supported path now that Step 5's frontend gate exists.
...
# enforcement is ON by default now — no "DRY RUN" line unless you've
# set RBAC_ENFORCE=false yourself
...
Since enforcement now defaults to on, requests from a role without
permission for a given module/action get a real `403` (or `401` if not
logged in) immediately — no separate step needed to "switch it on." If
you deliberately set `RBAC_ENFORCE=false` to watch decisions without
blocking anything (e.g. after changing `permission-matrix.js`), the
console logs `RBAC [DRY RUN] ... would block` for anything that would
have failed, with nothing actually blocked.
